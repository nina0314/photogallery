...
export function* fetchKeywordImages(action: PayloadAction<{ keyword: string; page?: number }>): Generator<any, void, any> {
  try {
    
    const page: number = yield select((state: RootState) => state.image.page);
    ...
    const res = yield call(
      fetch,
      `https://api.unsplash.com/search/photos?query=${action.payload.keyword}&client_id=${clientKey}&per_page=50&page=${page}`
    );
